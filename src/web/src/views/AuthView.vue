...
async function submit() {
  ...
  try {
    if (mode.value === "register") {
      const loggedIn = await auth.register(email.value, password.value, registrationRole.value);
      if (loggedIn) await router.push(`/app/${auth.role}/dashboard`);
      else success.value = "注册申请已受理，请打开验证邮件完成注册。";
...
<template>
  <header class="topbar"><RouterLink class="brand-button" to="/"><img class="brand-mark" src="/favicon.svg" alt="" aria-hidden="true" />Purslyx</RouterLink><span class="topbar-spacer" /><RouterLink class="button soft" :to="mode === 'login' ? '/app/register' : '/app/login'">{{ mode === "login" ? "创建账号" : "返回登录" }}</RouterLink></header>
