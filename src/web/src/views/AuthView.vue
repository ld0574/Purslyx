...
<template>
  ...
  <main class="public-main"><section class="auth-layout"><div class="auth-story"><div class="eyebrow">YOUR NEXT OPPORTUNITY</div><h1>先确认事实，<span>再生成下一步。</span></h1><p>身份在注册时固定。求职与招聘使用相同的证据规则，数据、版本和用量按账号隔离。</p><div class="auth-points"><div class="auth-point"><div><strong>资料可恢复</strong><small>重新登录后继续未完成任务</small></div></div><div class="auth-point"><div><strong>计次可审计</strong><small>成功只结算一次，失败释放预留</small></div></div></div></div>
    <form id="auth-form" class="auth-card" @submit.prevent="submit"><h2>{{ { login: "登录工作台", register: "创建账号", forgot: "找回密码", reset: "重置密码", verify: "验证邮箱" }[mode] }}</h2><p>本机运行连接《本地开发环境.md》的 201 PostgreSQL。</p><div v-if="mode === 'register'" class="identity-grid" style="margin-top:20px"><label class="identity-card" :class="{active: registrationRole === 'seeker'}"><input v-model="registrationRole" type="radio" name="registration_role" value="seeker" /><span><strong>求职者</strong><small>匹配、改写、PDF 与面试</small></span></label><label class="identity-card" :class="{active: registrationRole === 'recruiter'}"><input v-model="registrationRole" type="radio" name="registration_role" value="recruiter" /><span><strong>招聘方</strong><small>单人分析与待核实问题</small></span></label></div><div class="auth-fields" style="margin-top:20px"><div v-if="['login','register','forgot'].includes(mode)" class="field-group"><label for="auth-email">邮箱</label><input id="auth-email" v-model="email" class="field" type="email" autocomplete="email" required /></div><div v-if="['login','register','reset'].includes(mode)" class="field-group"><label for="auth-password">{{ mode === 'reset' ? '新密码' : '密码' }}</label><input id="auth-password" v-model="password" class="field" type="password" minlength="12" autocomplete="current-password" required /></div><div v-if="['verify','reset'].includes(mode)" class="field-group"><label for="auth-token">一次性令牌</label><input id="auth-token" v-model="token" class="field" required /></div></div><AsyncState :loading="loading" :error="error" :success="success" /><button class="button primary full" type="submit" style="margin-top:18px">确认</button><div class="auth-foot"><RouterLink v-if="mode === 'login'" to="/app/forgot-password">忘记密码</RouterLink> · 状态变更请求经过 CSRF、频率与幂等校验。</div></form>
  </section></main>
</template>
