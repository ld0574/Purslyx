...
async function createRewrite() {
  ...
  try {
    ...
    rewrite.value = result.rewrite; success.value = "逐段改写已经生成";
...
<template><AppShell><PageHeader eyebrow="FACTS & REWRITE" title="事实与改写" description="先补充本人确认的事实，再按段生成建议；生成与采用分开。"><button class="button outline small" @click="load">刷新</button></PageHeader><AsyncState :loading="loading" :error="error" :success="success" />
  ...
  <section v-if="rewrite" class="card" style="margin-top:18px"><div class="card-head"><div><h3>逐段改写对照</h3><p>原文、建议、理由和依据始终一起展示。</p></div><div class="item-actions"><span class="tag success">{{ rewrite.status }}</span><button class="button link-button small" :disabled="busy" type="button" @click="deleteRewrite">删除改写</button></div></div><div class="card-body stack"><article v-for="segment in rewrite.segments || []" :key="segment.id" class="report-card"><div class="grid-2"><div><div class="eyebrow">原始表达</div><p style="margin-top:10px;line-height:1.7">{{ segment.original_text }}</p></div><div><div class="eyebrow">建议表达 · 可编辑</div><textarea v-model="segment.edited_text" class="textarea" style="min-height:120px" :placeholder="segment.suggested_text" /></div></div><div class="callout">{{ segment.rationale }}</div><div v-for="evidence in segment.evidence || []" :key="evidence.id" class="evidence">{{ evidence.quote }}</div><div class="item-actions" style="margin-top:14px"><button class="button soft small" @click="decide(segment, 'keep_original')">保留原文</button><button class="button outline small" @click="decide(segment, 'edited')">保存编辑</button><button class="button primary small" @click="decide(segment, 'adopt')">采用建议</button><button v-if="segment.current_decision === 'adopt'" class="button link-button small" @click="decide(segment, 'revert')">撤回采用</button><span class="tag neutral">{{ segment.current_decision || '待确认' }}</span></div></article><div class="item-actions"><RouterLink class="button primary" :to="{ path: '/app/seeker/variants', query: { analysis_id: analysis.id, rewrite_id: rewrite.id } }">进入岗位版简历</RouterLink></div></div></section></template>
</AppShell></template>
