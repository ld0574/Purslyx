...
| --- | --- | --- |
| `admin_audit_events` | 管理操作审计事件表 | 保存后台变更的操作者、目标、结果和白名单前后值。 |
| `security_events` | 登录安全事件表 | 保存登录、限频、会话、验证和账号恢复等安全事实。 |
| `log_exports` | 日志导出表 | 保存授权日志导出的筛选、任务、文件和期限。 |
| `integrity_scan_runs` | 完整性巡检执行表 | 保存每日只读引用巡检的范围、状态和汇总。 |
| `integrity_scan_findings` | 完整性巡检异常表 | 保存孤儿、跨账号或删除状态异常的待复核发现。 |
...
| target_type | VARCHAR(64) | 否 | — | — | 账号、角色、权限、发放、反馈、日志导出等受控类型 |
| target_id | BIGINT | 是 | — | — | 目标内部 ID；创建失败且尚无目标时可空 |
| request_id | UUID | 否 | — | — | 关联一次 HTTP/命令请求 |
...
| correlation_id | UUID | 否 | — | — | 跨请求、任务与日志关联标识 |
...
| web_session_id | BIGINT | 是 | — | — | 可选引用 `web_sessions.id` |
| browser_session_id | BIGINT | 是 | — | — | 可选引用 `browser_sessions.id` |
| event_type | SMALLINT | 否 | — | CK: `event_type BETWEEN 1 AND 9` | 登录、限频、锁定、重置、会话签发/撤销等 |
| result | SMALLINT | 否 | — | CK: `result IN (1,2,3)` | 成功、拒绝或失败 |
| reason_code | VARCHAR(64) | 是 | — | — | 稳定原因码 |
| email_digest | BYTEA | 是 | — | — | 账号未知时使用规范化邮箱的带盐摘要 |
| ip_prefix | INET | 是 | — | — | 降精度网络前缀，不保存完整客户端 IP |
| ip_digest | BYTEA | 是 | — | — | 带轮换盐的完整 IP 摘要 |
| user_agent_digest | BYTEA | 是 | — | — | User-Agent 摘要 |
| request_id | UUID | 否 | — | — | 请求关联标识 |
| correlation_id | UUID | 否 | — | — | 跨日志关联标识 |
...
| output_file_id | BIGINT | 是 | — | — | 成功后引用私有 `stored_files.id` |
...
| deleted_at | TIMESTAMPTZ | 是 | — | — | 提前删除后立即不可下载 |
...
| source_table | VARCHAR(63) | 否 | — | — | 受控来源表名 |
| source_record_id | BIGINT | 否 | — | — | 来源记录内部 ID |
| source_account_id | BIGINT | 是 | — | — | 来源记录账号 ID；全局表可空 |
| reference_column | VARCHAR(63) | 否 | — | — | 被检查的逻辑引用列 |
| target_table | VARCHAR(63) | 否 | — | — | 受控目标表名 |
| target_record_id | BIGINT | 否 | — | — | 引用中记录的目标 ID |
| target_account_id | BIGINT | 是 | — | — | 可找到目标时的账号 ID |
...
| reviewed_by_account_id | BIGINT | 是 | — | — | 引用复核管理员账号 |
...
| admin_audit_events | idx_admin_audit_actor_created | `actor_account_id, created_at DESC` | B-tree | 管理操作按操作者与时间筛选 |
...
| security_events | idx_security_events_account_created | `account_id, created_at DESC` | B-tree partial | `account_id IS NOT NULL`；安全日志按账号与时间筛选 |
...
| log_exports | idx_log_exports_requester_created | `requested_by_account_id, created_at DESC` | B-tree | 申请人读取自己的导出历史 |
...
| integrity_scan_findings | idx_integrity_findings_run | `integrity_scan_run_id, id` | B-tree | 按一次巡检读取异常列表 |
...
- 邮箱/IP 摘要、请求与跨日志 correlation、会话引用、审计目标、导出文件引用、巡检运行状态及异常动态来源／目标只用于已限定时间窗、导出记录或巡检批次内复核，不单独建索引。管理日志主要筛选、安全事件主要筛选、导出到期清理和未关闭巡检异常保留必要索引。
