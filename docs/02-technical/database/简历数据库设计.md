...
| file_purpose | SMALLINT | 否 | — | CK: `file_purpose IN (1,2,3)` | 导入原件、岗位版 PDF 或临时转换文件 |
| storage_key | TEXT | 否 | — | UK | 私有存储逻辑键，不保存对外 URL 或服务器绝对路径 |
...
| account_id | BIGINT | 否 | — | Index | 引用 `accounts.id`，应用层校验 |
| source_file_id | BIGINT | 是 | — | Index | 引用 `stored_files.id`；纯文本导入时为空 |
...
| source_file_id | BIGINT | 是 | — | Index | 引用本次解析的 `stored_files.id` |
...
| expires_at | TIMESTAMPTZ | 是 | — | Index | 未确认临时草稿的清理时间点 |
...
| account_id | BIGINT | 否 | — | Index | 所属账号 ID |
| document_id | BIGINT | 否 | — | Index | 引用 `documents.id`，应用层校验 |
| source_draft_id | BIGINT | 是 | — | Index | 引用已确认 `document_drafts.id` |
...
| is_default | BOOLEAN | 否 | false | Index | 本人期望中每账号最多一条默认值 |
| status | SMALLINT | 否 | 1 | CK: `status IN (1,2)` | 启用或停用 |
| revision | BIGINT | 否 | 1 | CK: `revision >= 1` | 父记录更新乐观锁版本 |
| created_at | TIMESTAMPTZ | 否 | now() | — | 创建时间点 |
| updated_at | TIMESTAMPTZ | 否 | now() | — | 最后更新时间点 |
| deleted_at | TIMESTAMPTZ | 是 | — | Index | 删除时间点；历史分析仍保留版本引用但不可由用户打开已删期望 |
...
| created_at | TIMESTAMPTZ | 否 | now() | — | 创建时间点 |
| updated_at | TIMESTAMPTZ | 否 | now() | — | 最后更新时间点 |
| deleted_at | TIMESTAMPTZ | 是 | — | Index | 用户删除时间点 |
...
| source_document_version_id | BIGINT | 是 | — | Index | 可选引用原资料 `document_versions.id` |
...
| source_resume_version_id | BIGINT | 否 | — | Index | 引用实际使用的 `document_versions.id` |
| preference_version_id | BIGINT | 是 | — | Index | 引用本次分析期望版本 |
...
| deleted_at | TIMESTAMPTZ | 是 | — | Index | 删除后立即不可访问 |
...
| fact_version_id | BIGINT | 是 | — | Index | 可选引用 `resume_fact_versions.id` |
| document_version_id | BIGINT | 是 | — | Index | 可选引用 `document_versions.id` |
...
| source_resume_version_id | BIGINT | 否 | — | Index | 引用作为起点的 `document_versions.id` |
...
| created_at | TIMESTAMPTZ | 否 | now() | Index | 创建时间点 |
| updated_at | TIMESTAMPTZ | 否 | now() | — | 最后更新时间点 |
| deleted_at | TIMESTAMPTZ | 是 | — | Index | 删除后版本和导出立即不可访问 |
...
| rewrite_id | BIGINT | 是 | — | Index | 可选引用采用建议来源 `resume_rewrites.id` |
...
| output_file_id | BIGINT | 是 | — | Index | 成功后引用 `stored_files.id` |
...
| deleted_at | TIMESTAMPTZ | 是 | — | Index | 删除后立即禁止下载 |
...
| stored_files | uq_stored_files_storage_key | `storage_key` | Unique | 防止多个元数据指向同一逻辑键 |
| stored_files | idx_stored_files_account_digest | `account_id, sha256_digest` | B-tree | 同账号复用、归属校验和巡检 |
...
| documents | idx_documents_source_file | `account_id, source_file_id` | B-tree partial | `source_file_id IS NOT NULL`；文件引用校验与巡检 |
...
| document_drafts | idx_document_drafts_document_created | `account_id, document_id, created_at DESC` | B-tree | 资料草稿历史并覆盖两个引用 |
| document_drafts | idx_document_drafts_source_file | `account_id, source_file_id` | B-tree partial | `source_file_id IS NOT NULL`；文件引用巡检 |
| document_drafts | idx_document_drafts_parse_task | `account_id, parse_task_id` | B-tree partial | `parse_task_id IS NOT NULL`；任务回写与巡检 |
| document_drafts | idx_document_drafts_expiry | `expires_at` | B-tree partial | `status IN (1,2) AND expires_at IS NOT NULL`；过期清理 |
...
| document_versions | idx_document_versions_account_document | `account_id, document_id, version_no DESC` | B-tree | 账号限定读取最新与历史版本 |
| document_versions | idx_document_versions_source_draft | `account_id, source_draft_id` | B-tree partial | `source_draft_id IS NOT NULL`；草稿引用巡检 |
...
| job_preferences | idx_job_preferences_account_context | `account_id, preference_context, created_at DESC` | B-tree partial | `deleted_at IS NULL`；期望列表与账号巡检 |
...
| job_preference_versions | idx_job_pref_versions_account_parent | `account_id, preference_id, version_no DESC` | B-tree | 账号限定读取与巡检 |
...
| resume_facts | idx_resume_facts_document | `account_id, document_id, created_at` | B-tree partial | `deleted_at IS NULL`；简历事实列表与引用巡检 |
...
| resume_fact_versions | idx_resume_fact_versions_account_fact | `account_id, fact_id, version_no DESC` | B-tree | 账号限定历史读取 |
| resume_fact_versions | idx_resume_fact_versions_source_doc | `account_id, source_document_version_id` | B-tree partial | `source_document_version_id IS NOT NULL`；来源巡检 |
...
| resume_rewrites | idx_resume_rewrites_resume_version | `account_id, source_resume_version_id` | B-tree | 来源版本巡检 |
| resume_rewrites | idx_resume_rewrites_preference | `account_id, preference_version_id` | B-tree partial | `preference_version_id IS NOT NULL`；期望版本巡检 |
...
| resume_rewrite_segments | idx_rewrite_segments_account_parent | `account_id, rewrite_id, position_no` | B-tree | 账号限定段落读取 |
...
| resume_rewrite_evidence | idx_rewrite_evidence_fact | `account_id, fact_version_id` | B-tree partial | `fact_version_id IS NOT NULL`；事实引用巡检 |
| resume_rewrite_evidence | idx_rewrite_evidence_document | `account_id, document_version_id` | B-tree partial | `document_version_id IS NOT NULL`；资料引用巡检 |
...
| resume_segment_decisions | idx_segment_decisions_account_parent | `account_id, rewrite_segment_id, decision_no DESC` | B-tree | 当前决策与采用历史 |
...
| resume_variants | idx_resume_variants_source_resume | `account_id, source_resume_version_id` | B-tree | 来源资料巡检 |
...
| resume_variant_versions | idx_variant_versions_account_parent | `account_id, variant_id, version_no DESC` | B-tree | 编辑历史读取 |
| resume_variant_versions | idx_variant_versions_rewrite | `account_id, rewrite_id` | B-tree partial | `rewrite_id IS NOT NULL`；改写来源巡检 |
...
| resume_exports | idx_resume_exports_variant | `account_id, variant_version_id, created_at DESC` | B-tree partial | `deleted_at IS NULL`；版本导出列表与巡检 |
| resume_exports | idx_resume_exports_output_file | `account_id, output_file_id` | B-tree partial | `output_file_id IS NOT NULL`；文件引用和清理 |
| resume_exports | idx_resume_exports_created | `account_id, created_at DESC` | B-tree partial | `deleted_at IS NULL`；导出历史 |

## 5. 检查约束清单

| 表名 | 约束名 | 表达式 | 说明 |
| --- | --- | --- | --- |
| stored_files | ck_stored_files_purpose | `file_purpose IN (1,2,3)` | 文件用途合法 |
...
| job_preference_versions | ck_job_pref_versions_content_cleanup | `content_cleared_at IS NULL OR (job_title IS NULL AND location_text IS NULL AND work_mode IS NULL AND salary_min IS NULL AND salary_max IS NULL AND salary_currency IS NULL AND salary_period IS NULL AND salary_months IS NULL)` | 标记清理完成后期望值必须为空 |
...
JSONB 只接受版本化 Schema：`document-draft-v1`、`missing-fields-v1`、`document-content-v1`、`resume-variant-content-v1` 和 `resume-layout-v1`。Schema 文件与 Pydantic 模型同版本提交；读取旧版本时使用对应解析器，升级生成新资料或岗位版版本，不能原地重写历史。改写结果还固定 `rule_version`、`prompt_version`；PDF 固定 `template_version`、字体包版本和渲染镜像摘要。
...
- 上线前需用真实中文 DOC、DOCX、文字型 PDF 样本确定单文件大小、页数、正文长度和解析超时上限。
- `resume-layout-v1` 允许的字体、字号、间距范围及固定字体包版本需在 PDF 渲染实现前冻结。
- 私有资料、成品 PDF 和解析失败草稿的具体保留/归档期限需结合隐私告知与磁盘容量另行确认。
