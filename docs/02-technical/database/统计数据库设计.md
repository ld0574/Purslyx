...
| --- | --- | --- |
| `product_feedback` | 产品反馈表 | 保存用户主动提交的体验、评分和付费意愿。 |
| `daily_metric_rollups` | 每日指标聚合表 | 保存按日期、身份和功能生成的可重算站点指标。 |
...
| context_resource_id | BIGINT | 是 | — | — | 可选受控业务资源内部 ID |
...
| revision | BIGINT | 否 | 1 | CK: `revision >= 1` | 管理处理反馈时的乐观锁版本 |
| reviewed_by_account_id | BIGINT | 是 | — | — | 引用有反馈查看权限的管理员账号 |
| reviewed_at | TIMESTAMPTZ | 是 | — | — | 首次查看时间点 |
| created_at | TIMESTAMPTZ | 否 | now() | Index | 提交时间点 |
| updated_at | TIMESTAMPTZ | 否 | now() | — | 状态更新时间点 |
| deleted_at | TIMESTAMPTZ | 是 | — | — | 用户删除后正文立即不可访问 |
...
| product_feedback | idx_product_feedback_account_created | `account_id, created_at DESC` | B-tree partial | `deleted_at IS NULL`；管理端按账号读取反馈历史 |
...
| product_feedback | ck_product_feedback_revision | `revision >= 1` | 反馈处理并发版本为正数 |
...
- 反馈复核人和上下文只作为已定位反馈的审计字段，每日聚合水位从当前规则范围直接读取；反馈删除后的低频正文清理由主键分段扫描，这些路径不单独建索引。管理反馈列表、账号筛选和按范围读取趋势保留必要索引。
