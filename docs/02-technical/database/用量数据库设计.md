...
全部连线均为应用层校验的逻辑引用，不创建数据库外键。全局价格版本和预算桶没有用户归属；写入仍须事务校验，反向对账由现有预算桶、任务和调用索引完成，不为只参与写入校验的价格引用另建索引。
...
| --- | --- | --- |
| `usage_balances` | 功能用量余额表 | 保存账号按适用功能汇总的发放、预留和结算数量。 |
| `usage_grants` | 用量发放表 | 保存试用或管理员追加次数的不可变发放事实。 |
| `usage_reservations` | 用量预留表 | 保存计次业务任务的预留、结算或释放生命周期。 |
| `usage_ledger_entries` | 用量流水表 | 追加保存所有余额变动，供展示和余额重放。 |
| `model_price_versions` | 模型价格版本表 | 保存供应商模型价格、生效区间与核验来源。 |
| `site_budget_buckets` | 全站日预算桶表 | 保存上海自然日的美元预算、占用与并发槽汇总。 |
| `budget_reservations` | 预算预留表 | 保存单次模型调用的成本上界、实际成本和未知占用。 |
...
| created_at | TIMESTAMPTZ | 否 | now() | — | 预留时间点 |
...
| account_id | BIGINT | 否 | — | Index | 所属账号 ID |
| usage_balance_id | BIGINT | 否 | — | — | 引用 `usage_balances.id` |
| usage_grant_id | BIGINT | 是 | — | — | 发放事件引用 `usage_grants.id` |
| usage_reservation_id | BIGINT | 是 | — | Index | 预留相关事件引用 `usage_reservations.id` |
| async_task_id | BIGINT | 是 | — | — | 计次业务任务 ID |
...
| effective_to | TIMESTAMPTZ | 是 | — | — | 生效终点；当前版本为空 |
...
| account_id | BIGINT | 否 | — | — | 调用所属账号 ID |
| async_task_id | BIGINT | 否 | — | — | 调用所属任务 ID |
| model_price_version_id | BIGINT | 否 | — | — | 计算上界所用价格版本 |
...
| usage_grants | idx_usage_grants_account_created | `account_id, created_at DESC` | B-tree | 按账号读取发放历史 |
...
| usage_ledger_entries | idx_usage_ledger_account_created | `account_id, feature, created_at DESC` | B-tree | 用户用量流水分页 |
| usage_ledger_entries | idx_usage_ledger_reservation | `account_id, usage_reservation_id, id` | B-tree partial | `usage_reservation_id IS NOT NULL`；预留的结算／释放流水对账 |
...
| budget_reservations | idx_budget_reservations_bucket_status | `site_budget_bucket_id, status, reserved_at` | B-tree | 预算桶占用明细与结算 |
...
- 余额账号查询由 `(account_id, feature)` 唯一索引覆盖；试用批次、操作者、价格引用、发放流水关联、任务关联和账号预算预留历史不另建候选索引。余额重放直接按账号与功能读取用量流水，低频对账按主键分段扫描。账号发放历史、用量流水、预留生命周期、预留过期、预算桶占用和未知成本复核保留必要二级索引。

## 9. 待讨论项

- 首版试用批次键固定为 `internal-beta-v1`，仅在内测配置启用；通过内测注册入口且完成邮箱验证的账号按固定身份领取一次：求职分析 20 次、改写 20 次、面试 5 场，招聘分析 20 次。扩大名单或新建批次必须使用新的批次键，不能重复发放旧批次。
- 模型价格自动同步还是人工核验、账单差异如何形成更正事件，需在首次真实供应商联调后确定。（自动同步）
