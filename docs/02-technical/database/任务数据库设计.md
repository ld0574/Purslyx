...
| --- | --- | --- |
| `async_tasks` | 异步任务表 | 保存异步业务任务、状态、幂等键、租约和执行代次。 |
| `task_input_refs` | 任务输入引用表 | 保存任务创建时冻结的业务资源与版本引用。 |
| `task_outbox` | 任务发件箱表 | 保存事务提交后待发布到 Celery 的持久消息。 |
| `task_attempts` | 任务执行尝试表 | 保存每个执行代次的 Worker 租约和结果。 |
| `model_call_attempts` | 模型调用尝试表 | 保存每次模型调用的用量、成本、耗时和结果。 |
| `task_artifacts` | 任务产物表 | 保存可恢复步骤的结构化产物或文件引用。 |
| `workflow_checkpoint_refs` | 工作流检查点引用表 | 保存 LangGraph thread 与业务任务、资源及清理状态的映射。 |
...
| task_type | SMALLINT | 否 | — | CK: `task_type BETWEEN 1 AND 11` | 具体异步任务类型 |
...
| resource_id | BIGINT | 否 | — | — | 目标业务父资源内部 ID |
| resource_version_id | BIGINT | 是 | — | — | 目标不可变版本 ID；有版本的输入必须填写 |
...
| started_at | TIMESTAMPTZ | 否 | now() | — | 尝试开始时间点 |
...
| account_id | BIGINT | 否 | — | Index | 所属账号 ID |
| async_task_id | BIGINT | 否 | — | Index | 引用 `async_tasks.id` |
| task_attempt_id | BIGINT | 否 | — | — | 引用 `task_attempts.id` |
| model_price_version_id | BIGINT | 否 | — | — | 引用用量模块 `model_price_versions.id` |
...
| provider_request_id_digest | BYTEA | 是 | — | — | 供应商请求 ID 摘要 |
...
| account_id | BIGINT | 否 | — | Index | 所属账号 ID |
| async_task_id | BIGINT | 否 | — | Index | 引用 `async_tasks.id` |
| task_attempt_id | BIGINT | 否 | — | — | 引用产生该产物的尝试；仅保存生成谱系 |
...
| business_resource_id | BIGINT | 否 | — | — | 对应业务资源内部 ID |
...
| async_tasks | idx_async_tasks_account_created | `account_id, created_at DESC` | B-tree | 管理端任务日志按账号与时间筛选 |
...
| model_call_attempts | idx_model_calls_budget | `account_id, budget_reservation_id` | B-tree | 未知预算占用的调用对账 |
...
| async_tasks | ck_async_tasks_type | `task_type BETWEEN 1 AND 11` | 首版任务类型合法 |
...
| async_tasks.task_type | 11 | daily_rollup | 统计日聚合 | 免费系统任务，可按固定水位重算 |
...
- 输入版本、业务资源、outbox/尝试/产物的账号字段、产物尝试谱系和检查点业务资源不为巡检或低频删除反查单独建索引；它们通过任务唯一键、业务父记录或主键分段扫描定位。终态归档、价格版本和供应商请求复核也先使用分段扫描。任务派发、租约恢复、模型预算对账、账号成本历史、未知成本和正文清理保留必要二级索引。
