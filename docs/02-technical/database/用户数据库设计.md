...
| 更新日期 | 2026-09-14 |
| 状态 | 待评审；模型与迁移尚未实现 |
| 范围 | 固定注册身份账号、邮箱验证与密码重置、Web 会话、受限浏览器会话、后台角色与权限 |
...
- 规范化邮箱全局唯一；密码只存 Argon2id 摘要，验证、重置、Web 会话和浏览器会话只存随机秘密的摘要。
...
| token_type | SMALLINT | 否 | — | CK: `token_type IN (1,2)` | 邮箱验证或密码重置 |
| token_digest | BYTEA | 否 | — | UK | 随机令牌摘要；原令牌只发送给用户 |
| expires_at | TIMESTAMPTZ | 否 | — | Index | 令牌过期时间点 |
...
| nonce_digest | BYTEA | 否 | — | Index | Web 与脚本握手随机数摘要，防重放 |
| redirect_origin | VARCHAR(255) | 否 | — | — | 经白名单校验的产品域名来源 |
| expires_at | TIMESTAMPTZ | 否 | — | Index | 短期过期时间点 |
...
| granted_by_account_id | BIGINT | 否 | — | Index | 引用操作者 `accounts.id`，应用层校验后台权限 |
...
| assigned_by_account_id | BIGINT | 否 | — | Index | 引用授权人 `accounts.id`，应用层校验 |
| assigned_at | TIMESTAMPTZ | 否 | now() | — | 授权时间点 |
| revoked_by_account_id | BIGINT | 是 | — | Index | 引用撤销人 `accounts.id`，应用层校验 |
...
| account_tokens | idx_account_tokens_account_type_created | `account_id, token_type, created_at DESC` | B-tree | 撤销账号同类旧令牌与引用巡检 |
| account_tokens | idx_account_tokens_active_expiry | `expires_at` | B-tree partial | `consumed_at IS NULL AND revoked_at IS NULL`；过期清理 |
...
| web_sessions | idx_web_sessions_account_status_expiry | `account_id, status, expires_at` | B-tree | 账号会话列表、批量撤销与巡检 |
| web_sessions | idx_web_sessions_active_expiry | `expires_at` | B-tree partial | `status = 1`；过期清理 |
...
| browser_auth_codes | idx_browser_auth_codes_nonce | `nonce_digest` | B-tree | 握手随机数重放检测 |
| browser_auth_codes | idx_browser_auth_codes_account_expiry | `account_id, expires_at` | B-tree | 账号归属巡检与过期清理 |
...
| browser_sessions | idx_browser_sessions_account_status_expiry | `account_id, status, expires_at` | B-tree | 本账号会话读取、撤销和巡检 |
| browser_sessions | idx_browser_sessions_active_expiry | `expires_at` | B-tree partial | `status = 1`；过期清理 |
...
| admin_roles | idx_admin_roles_status_name | `status, display_name` | B-tree | 角色列表 |
...
| admin_role_permissions | idx_admin_role_permissions_permission | `permission_id, role_id` | B-tree | 按权限反查角色与巡检 |
| admin_role_permissions | idx_admin_role_permissions_grantor | `granted_by_account_id, created_at DESC` | B-tree | 授权人引用巡检与操作复核 |
...
| account_admin_roles | idx_account_admin_roles_account_active | `account_id, assigned_at DESC` | B-tree partial | `revoked_at IS NULL`；鉴权与账号详情 |
...
| account_admin_roles | idx_account_admin_roles_assigner | `assigned_by_account_id, assigned_at DESC` | B-tree | 授权人引用巡检 |
| account_admin_roles | idx_account_admin_roles_revoker | `revoked_by_account_id, revoked_at DESC` | B-tree partial | `revoked_by_account_id IS NOT NULL`；撤销人引用巡检 |
...
| account_tokens | ck_account_tokens_type | `token_type IN (1,2)` | 验证或重置令牌 |
...
| 暂停账号 | 校验 `admin.users.manage_status`；锁定账号，条件更新状态与版本，撤销全部有效会话并写管理操作日志 | 通知用户可异步重试 | `revision` 防并发覆盖；不能借此改变注册身份 |
...
- 上线前须确定邮箱验证、密码重置、Web 会话与浏览器会话的具体有效期；这些值属于受控配置，不写死在数据库约束中。
- 上线前须确定账号暂停的申诉与恢复流程；首版结构已支持恢复，但产品文案和管理权限尚需确认。
