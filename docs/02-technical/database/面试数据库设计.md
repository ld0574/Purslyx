...
| analysis_report_id | BIGINT | 否 | — | Index | 引用可查看 `analysis_reports.id` |
| resume_document_version_id | BIGINT | 否 | — | Index | 开场时冻结的简历版本 |
| job_document_version_id | BIGINT | 否 | — | Index | 开场时冻结的 JD 版本 |
| preference_version_id | BIGINT | 是 | — | Index | 开场时冻结的求职期望版本 |
...
| deleted_at | TIMESTAMPTZ | 是 | — | Index | 删除后会话全部内容立即不可访问 |
...
| account_id | BIGINT | 否 | — | Index | 所属账号 ID |
| interview_session_id | BIGINT | 否 | — | Index | 引用 `interview_sessions.id` |
| interview_question_id | BIGINT | 否 | — | Index | 引用本轮 `interview_questions.id` |
| interview_answer_id | BIGINT | 否 | — | Index | 引用本轮 `interview_answers.id` |
| async_task_id | BIGINT | 否 | — | UK | 引用本轮反馈任务 |
| status | SMALLINT | 否 | 1 | CK: `status IN (1,2,3)` | 生成中、可查看或失败 |
| feedback_content | JSONB | 是 | — | — | `interview-feedback-v1` 校验后的依据、遗漏与改进方向 |
...
| summary_content | JSONB | 是 | — | — | `interview-summary-v1` 校验后的优势、遗漏、改进和未完成项 |
...
| interview_sessions | idx_interview_sessions_account_status | `account_id, status, updated_at DESC` | B-tree partial | `deleted_at IS NULL`；会话列表、恢复和账号巡检 |
...
| interview_sessions | idx_interview_sessions_report | `account_id, analysis_report_id` | B-tree | 分析报告引用巡检 |
| interview_sessions | idx_interview_sessions_resume_version | `account_id, resume_document_version_id` | B-tree | 简历版本引用巡检 |
| interview_sessions | idx_interview_sessions_job_version | `account_id, job_document_version_id` | B-tree | JD 版本引用巡检 |
| interview_sessions | idx_interview_sessions_preference | `account_id, preference_version_id` | B-tree partial | `preference_version_id IS NOT NULL`；期望引用巡检 |
| interview_sessions | idx_interview_sessions_deleted | `deleted_at` | B-tree partial | `deleted_at IS NOT NULL`；清理编排 |
...
| interview_questions | idx_interview_questions_account_session | `account_id, interview_session_id, sequence_no` | B-tree | 账号限定问题读取 |
| interview_questions | idx_interview_questions_parent | `account_id, parent_question_id` | B-tree partial | `parent_question_id IS NOT NULL`；自引用巡检 |
| interview_questions | idx_interview_questions_task | `account_id, async_task_id` | B-tree | 任务回写与引用巡检 |
...
| interview_answers | idx_interview_answers_session | `account_id, interview_session_id, submitted_at` | B-tree | 会话恢复与会话引用巡检 |
...
| interview_feedback | idx_interview_feedback_session | `account_id, interview_session_id, created_at` | B-tree | 会话反馈读取与巡检 |
| interview_feedback | idx_interview_feedback_question | `account_id, interview_question_id` | B-tree | 问题引用巡检 |
| interview_feedback | idx_interview_feedback_answer | `account_id, interview_answer_id` | B-tree | 回答引用巡检 |
...
| interview_summaries | idx_interview_summaries_account_session | `account_id, interview_session_id, version_no DESC` | B-tree | 账号限定最新汇总与巡检 |
...
问题依据、反馈与汇总分别使用 `interview-question-basis-v1`、`interview-feedback-v1` 和 `interview-summary-v1`。`rule_version` 固定一场 3 个主问题、每题最多一次追问的解释；提示词升级只用于新生成记录。任务与模型调用版本从任务模块追溯。
...
- “需要追问”的模型结构化判定阈值与无追问时的用户提示需随 `interview-feedback-v1` 一起评审。
