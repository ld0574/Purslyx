...
flowchart LR
    ...
    AR -->|1:N analysis_report_id，同账号| AE[analysis_evidence]
    DS -->|0..1:N dimension_score_id，同账号| AE
...
| browser_session_id | BIGINT | 否 | — | Index | 引用有效 `browser_sessions.id` |
...
| job_document_version_id | BIGINT | 否 | — | Index | 引用已确认 JD `document_versions.id` |
| preference_version_id | BIGINT | 否 | — | Index | 引用入池时选择的完整期望版本 |
...
| source_url_digest | BYTEA | 是 | — | Index | 原链接规范化摘要 |
...
| resume_document_version_id | BIGINT | 否 | — | Index | 实际分析的简历版本 |
| job_document_version_id | BIGINT | 否 | — | Index | 实际分析的 JD 版本 |
| preference_version_id | BIGINT | 是 | — | Index | 本次求职期望或候选人明确期望版本 |
...
| deleted_at | TIMESTAMPTZ | 是 | — | Index | 删除后报告立即不可访问 |
...
| account_id | BIGINT | 否 | — | Index | 所属账号 ID |
| analysis_report_id | BIGINT | 否 | — | Index | 引用 `analysis_reports.id` |
| dimension_score_id | BIGINT | 是 | — | Index | 可选引用 `analysis_dimension_scores.id` |
| document_version_id | BIGINT | 否 | — | Index | 引用简历或 JD 的 `document_versions.id` |
...
| finding_type | SMALLINT | 否 | — | CK: `finding_type IN (1,2,3)` | 支持、待确认或差距 |
...
| account_id | BIGINT | 否 | — | Index | 所属求职账号 ID |
| job_pool_item_id | BIGINT | 否 | — | Index | 引用 `job_pool_items.id` |
| click_token | UUID | 否 | — | UK | 页面签发的一次性点击令牌 |
| platform | SMALLINT | 否 | — | CK: `platform IN (1,2)` | BOSS 直聘或猎聘 |
| target_url_digest | BYTEA | 否 | — | Index | 目标链接摘要，供核对与统计 |
...
| browser_job_drafts | idx_browser_job_drafts_session | `account_id, browser_session_id, created_at DESC` | B-tree | 会话权限校验与引用巡检 |
...
| job_pool_items | idx_job_pool_items_account_status | `account_id, analysis_status, created_at DESC` | B-tree partial | `deleted_at IS NULL`；匹配池列表与账号巡检 |
| job_pool_items | idx_job_pool_items_job_version | `account_id, job_document_version_id` | B-tree | JD 版本归属校验与巡检 |
| job_pool_items | idx_job_pool_items_preference | `account_id, preference_version_id` | B-tree | 期望版本归属校验与过期提示 |
| job_pool_items | idx_job_pool_items_source_url | `account_id, platform, source_url_digest` | B-tree partial | `source_url_digest IS NOT NULL AND deleted_at IS NULL`；来源去重与跳转检查 |
...
| analysis_reports | idx_analysis_reports_resume_version | `account_id, resume_document_version_id` | B-tree | 简历版本引用巡检 |
| analysis_reports | idx_analysis_reports_job_version | `account_id, job_document_version_id` | B-tree | JD 版本引用巡检 |
| analysis_reports | idx_analysis_reports_preference | `account_id, preference_version_id` | B-tree partial | `preference_version_id IS NOT NULL`；期望引用巡检 |
...
| analysis_dimension_scores | idx_dimension_scores_account_report | `account_id, analysis_report_id, dimension_code` | B-tree | 账号限定读取和巡检 |
| analysis_evidence | pk_analysis_evidence | `id` | Primary | 主键 |
| analysis_evidence | idx_analysis_evidence_report | `account_id, analysis_report_id, id` | B-tree | 报告全部依据与引用巡检 |
| analysis_evidence | idx_analysis_evidence_dimension | `account_id, dimension_score_id` | B-tree partial | `dimension_score_id IS NOT NULL`；维度依据读取 |
| analysis_evidence | idx_analysis_evidence_document | `account_id, document_version_id, source_segment_key` | B-tree | 资料版本和段落引用校验 |
...
| analysis_condition_results | idx_condition_results_account_report | `account_id, analysis_report_id, condition_code` | B-tree | 账号限定条件读取与巡检 |
...
| apply_click_events | idx_apply_click_events_job | `account_id, job_pool_item_id, created_at DESC` | B-tree | 岗位点击历史与引用巡检 |
...
| apply_click_events | idx_apply_click_events_target | `account_id, target_url_digest` | B-tree | 链接审计与异常检查 |
...
| analysis_evidence | ck_analysis_evidence_finding | `finding_type IN (1,2,3)` | 依据结论合法 |
...
| 发布分析结果 | 锁定任务、报告和来源父资源；校验当前执行代次、未删除、4 个维度唯一且权重合计为 1、条件结果完整；写结果并标可用 | 通知或页面轮询可重试 | 同任务最多一份可用报告；迟到执行不能覆盖或结算 |
...
- 能力维度、原文证据与条件对照拆表，前端可以逐项展示，统计也无需解析报告大 JSON；整体建议保留受 Schema 校验的可变结构。
