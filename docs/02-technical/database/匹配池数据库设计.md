...
- 浏览器脚本只在求职者打开受支持 JD 详情页后获取当前岗位；上传事务直接创建私有岗位和 JD 版本，不调用模型、不绑定简历／期望、不预留分析次数。相同账号、平台、规范化 URL 摘要和内容指纹幂等去重。
- 用户点击匹配后才选择本次简历、岗位期望并确认分析消耗；简历、期望和余额满足时，预留、任务与 outbox 在同一事务提交，岗位随后逐条分析。手动 JD 也只保存岗位本身，遵循同一匹配入口。
...
| --- | --- | --- |
| `browser_job_drafts` | 浏览器岗位草稿表 | 保存 BOSS 与猎聘页面上传的短期来源记录和去重指纹。 |
| `job_pool_items` | 匹配池岗位表 | 保存求职账号直接入池的私有岗位及后续分析输入版本。 |
...
| match_coefficient | NUMERIC(5,4) | 否 | — | CK: `match_coefficient BETWEEN 0 AND 1` | 模型逐条 0—100 分经证据校验后的代码评分系数 |
...
| job_pool_items | uq_job_pool_items_browser_draft | `account_id, source_browser_draft_id` | Unique partial | `source_browser_draft_id IS NOT NULL AND deleted_at IS NULL`；一个账号的草稿最多保留一个未删除岗位 |
...
| analysis_requirement_results | ck_analysis_requirement_results_scoring | `match_coefficient BETWEEN 0 AND 1` 且 `needs_confirmation` 不计入覆盖；`gap` 系数为 0；有证据的支持／部分支持采用经校验的逐条细分分 | 要求状态、评分系数和覆盖标记一致 |
...
| analysis_requirement_results.finding_type / analysis_evidence.finding_type | 4 | partially_supported | 部分支持 | 评分系数由逐条 0—100 贴合度转换，必须有可核对依据 |
| analysis_requirement_results.match_coefficient | 0 | unsupported_or_unknown | 未贡献已证实匹配分 | `gap` 为明确差距；`needs_confirmation` 为未知且不计覆盖 |
| analysis_requirement_results.match_coefficient | 0—1 | calibrated | 细分贴合度 | 由模型逐条 0—100 分转换，服务端只接受有效原文引用后写入 |
...
| --- | --- | --- | --- |
| 浏览器上传并直接入池 | 校验浏览器会话有效、账号为求职身份、平台与 URL 白名单；按内容唯一键插入草稿，并在同一事务创建确认 JD 版本和 `job_pool_items` | 脚本提示“岗位信息获取完成”；服务端过期后清理来源草稿 | 相同内容返回原岗位；不绑定输入、不预留次数、不创建任务 |
| 点击匹配但暂不具备条件 | 锁定账号、岗位、简历版本与期望版本；条件不足或余额不足时保留岗位 | 页面展示缺失条件和恢复入口 | 不创建模型任务，不消耗次数，岗位继续可见 |
| 点击匹配并分析 | 按账号、输入版本、用量余额顺序锁定；校验简历、JD、期望与身份；预留 1 次分析、任务、报告占位与 outbox | Worker 读取冻结版本分析 | 同幂等键返回原任务；失败释放预留并保留岗位 |
