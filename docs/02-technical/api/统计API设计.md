...
需要 Web 会话。`days` 仅允许 `7`、`14`、`30`，默认 14；返回当前账号的精确计数、适用功能余额、待处理状态、逐日业务趋势和面试练习趋势，日期按 `Asia/Shanghai` 计算。求职端的投递指标固定命名为“去投递点击”，不得解释为外部平台已提交成功；练习趋势只纳入 `interview-rubric-v1` 下完整完成的三道主问题。
