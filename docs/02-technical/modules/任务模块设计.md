...
必须覆盖重复业务提交、同键不同摘要、outbox 补投、重复消息、并发认领、Worker 崩溃与租约到期、旧代次迟到、供应商结果未知、检查点恢复、来源删除、次数只结算一次、预算槽释放和免费任务不动余额。201 演示已实现 `claim_outbox_batch`、`claim_task_execution`、`heartbeat_task`、`recover_expired_leases` 和八类本地任务 Handler；PostgreSQL 并发竞争、压力和正式 Celery/Redis 适配仍待验收。
