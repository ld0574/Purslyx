...
| 版本 | v0.1 |
| 更新日期 | 2026-09-14 |
| 状态 | 已实现并通过 Worker 崩溃恢复、失败重试和迟到写回验收 |
...
| `OutboxDispatcher` | PostgreSQL outbox 小批量认领；生产环境可替换为 Celery 发布器 |
| `WorkerRunner` | `src/server/app/worker.py` 按 task_id 认领租约、恢复步骤、续租并调用业务 Worker |
...
必须覆盖重复业务提交、同键不同摘要、outbox 补投、重复消息、并发认领、Worker 崩溃与租约到期、旧代次迟到、供应商结果未知、检查点恢复、来源删除、次数只结算一次、预算槽释放和免费任务不动余额。201 PostgreSQL 环境已验证 `claim_outbox_batch`、`claim_task_execution`、`heartbeat_task`、`recover_expired_leases`、八类本地任务 Handler、并发竞争和租约恢复；压力测试和正式 Celery/Redis 适配仍待验收。

测试使用 PostgreSQL 18 和真实事务隔离语义；队列与模型可用可控替身，另用小样本真实调用验证用量字段和错误分类。上线前仍需冻结任务类型注册表、输入引用类型、租约／心跳／退避参数、业务错误目录和 LangGraph 检查点清理协议。
